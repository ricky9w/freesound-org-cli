...
export function networkError(error: unknown): AppError {
  ...
  if (code && /CERT|SSL|TLS|VERIFY/.test(code))
    return new AppError(
      "TLS_ERROR",
      "Freesound HTTPS certificate validation failed.",
      `TLS error: ${code}. Keep certificate verification enabled; retry after the certificate or trust-chain issue is resolved.`,
    );
...
export const interactive = () =>
  !!process.stdin.isTTY && !!process.stderr.isTTY && !machine();
export function output(data: unknown) {
  if (machine())
    console.log(JSON.stringify({ schemaVersion: 1, ok: true, data }));
    ...
    return data.results
      .map(
        (x: Record<string, unknown>) =>
          `${x.id ?? ""}  ${clean(String(x.name ?? ""))}  ${x.duration ?? ""}s`,
      )
      .join("\n");
  }
  ...
  const e =
    error instanceof AppError
      ...
      : new AppError(
          ...
          "Run the command again with --help; credentials are never included in diagnostics.",
        );
  if (machine())
    console.log(
      JSON.stringify({
...
export const configDir = () =>
  process.env.FSND_CONFIG_DIR ||
  join(process.env.XDG_CONFIG_HOME || join(homedir(), ".config"), "fsnd");
...
export async function locked<T>(
  ...
  if (!handle)
    throw new AppError(
      ...
      "If it crashed, remove the stale auth.lock file after confirming no fsnd process is active.",
    );
  try {
    return await task();
  } finally {
