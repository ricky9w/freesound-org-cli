...
export function apiURL(path: string) {
  ...
  if (
    ...
    throw new AppError(
      "INVALID_PATH",
      "Use an API resource path, for example sounds/123/.",
    );
  if (url.searchParams.has("token"))
    throw new AppError(
      "INVALID_PATH",
      "Pass credentials through configuration, not URLs.",
    );
  return url;
}
async function request(url: URL, header: string): Promise<Response> {
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      r = await fetch(url, {
        headers: { Authorization: header, "User-Agent": "fsnd/0.1.0" },
        redirect: "error",
    ...
    if ((r.status === 429 || r.status >= 500) && attempt < 2) {
      ...
      if (!Number.isFinite(seconds) || seconds > 8) {
        ...
        throw new AppError(
          "RATE_LIMITED",
          "Freesound asked us to wait before another request.",
          "Try again later.",
          r.status,
        );
      }
      ...
      throw new AppError(
        r.status === 401
          ...
          : r.status === 429
            ? "RATE_LIMITED"
            : "API_ERROR",
        `Freesound returned HTTP ${r.status}.`,
        r.status === 401 ? "Run fsnd auth status or log in again." : undefined,
        r.status,
      );
    }
    return r;
...
export async function api(
  ...
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new AppError(
      "INVALID_RESPONSE",
      "Expected a JSON object from Freesound.",
    );
...
export async function search(
  ...
  for (let i = 0; i < options.pages; i++) {
    last = await api("search/text/", options.mode, {
      query,
      filter: options.filter,
      sort: options.sort,
      fields:
        options.fields ||
        "id,name,username,duration,license,tags,previews,type",
      page: options.page + i,
      page_size: options.pageSize,
    });
    if (!Array.isArray(last.results))
      throw new AppError("INVALID_RESPONSE", "Search results are missing.");
...
export function validateDownloadURL(url: URL) {
  if (
    ...
    throw new AppError(
      "DOWNLOAD_HOST",
      "The download points outside the supported Freesound hosts.",
    );
}
export async function downloadResponse(
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (!r.ok) {
      ...
      throw new AppError(
        "DOWNLOAD_FAILED",
        `Download returned HTTP ${r.status}.`,
        undefined,
        r.status,
      );
    }
...
export async function saveResponse(response: Response, output: string) {
  ...
  if (!response.body || /json|text\/|html/.test(contentType)) {
    await response.body?.cancel();
    throw new AppError("INVALID_AUDIO", "The server did not return audio.");
  }
  let bytes = 0;
  ...
  try {
    ...
    if (!bytes)
      throw new AppError("EMPTY_AUDIO", "The server returned an empty file.");
...
export async function download(
  ...
  const target = preview
    ...
    : `${API}sounds/${id}/download/`;
  if (!target)
    throw new AppError("PREVIEW_UNAVAILABLE", "No MP3 preview is available.");
  const auth = preview ? undefined : await authorization(mode, true);
  const saved = await saveResponse(
    await downloadResponse(new URL(target), auth),
    file,
  );
  await privateJson(metadataFile, {
    schemaVersion: 1,
    source: "freesound",
    soundId: id,
    preview,
