...
The public npm package is `fsnd`. `.github/workflows/publish.yml` publishes when a `v*` tag is pushed. Normal branch pushes only run CI. The tag must equal `v` plus the package version; all checks and packaged Node/Bun smoke checks must pass before publication.

One-time setup after the initial package exists:

```sh
npm trust github fsnd --repo ricky9w/freesound-org-cli --file publish.yml --allow-publish --yes
```
...
Inspect the Publish run and `npm view fsnd@X.Y.Z dist.attestations --json`; install from the registry and run `npm audit signatures` to verify attestations. Published versions are immutable. If a run fails, inspect whether the registry already contains the version before retrying.
